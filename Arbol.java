...
public class Arbol
{
	...
	{
		if(dato==nodoAux.getDato()) return;
	...
	public nodoArbol buscar(int dato, nodoArbol nodoAux)
	{
		...
		if(dato==raiz.getDato()) return raiz;
		return padre;
	}
		...
		if(nodoAux!=null)
		{
			...
			else
			{
				if(nodoAux.getDerecho().isTieneHijos())
				{
					...
					else if(nodoAux.getDerecho().getDerecho()==null && nodoAux.getDerecho().getIzquierdo
							()!=null)
